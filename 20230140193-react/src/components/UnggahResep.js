    ...
    if (success) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-mesh relative overflow-hidden p-6 font-sans">
                <Particles count={30} opacity={0.2} speed={0.3} color="#06b6d4" />
                <div className="absolute inset-0 bg-dot-pattern opacity-5 pointer-events-none"></div>
                <div className="max-w-md w-full glass-card p-12 text-center shadow-premium border border-slate-100 animate-fade-in relative z-10">
                    <div className="w-24 h-24 bg-lime-500/10 border border-lime-500/20 rounded-[32px] flex items-center justify-center mx-auto mb-8 shadow-inner shadow-lime-500/10">
                        <CheckCircle className="w-12 h-12 text-lime-600" strokeWidth={2.5} />
                    </div>
                    <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tighter">Resep Terkirim!</h2>
                    ...
                    <div className="p-5 bg-cyan-500/5 rounded-2xl border border-cyan-500/10 flex items-center justify-center gap-4 transition-all">
                        <div className="w-2.5 h-2.5 bg-cyan-500 rounded-full animate-pulse shadow-lg"></div>
                        <p className="text-[10px] font-black text-cyan-600 uppercase tracking-[0.3em]">ESTIMASI RESPON: 15-30 MENIT</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-mesh font-sans text-slate-900 overflow-x-hidden relative">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute top-[-10%] right-[-10%] w-[60%] h-[60%] bg-cyan-500/5 rounded-full blur-[150px] animate-pulse-glow"></div>
                <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-lime-500/5 rounded-full blur-[150px] animate-pulse-glow" style={{ animationDelay: '-3s' }}></div>
            </div>
                ...
                <div className="container mx-auto px-6">
                    <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
                        ...
                        <div className="animate-fade-in">
                            <div className="inline-flex items-center gap-2 bg-cyan-500/5 border border-cyan-500/10 px-4 py-1.5 rounded-full text-cyan-600 text-[10px] font-black uppercase tracking-[0.2em] mb-8">
                                <Package size={14} /> APOTEK HADINATA
                            </div>
                            <h2 className="text-4xl md:text-6xl font-black text-slate-900 leading-tight mb-8 tracking-tighter">
                                Layanan <span className="text-cyan-500 underline decoration-cyan-500/10 scale-110">Resep Digital</span> Terpercaya.
                            </h2>
                            <p className="text-lg text-slate-600 font-bold leading-relaxed mb-12">
                            ...
                            <div className="space-y-8">
                                {[
                                    ...
                                    <div key={i} className="flex gap-6 group">
                                        <div className="w-14 h-14 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-center text-cyan-500 group-hover:scale-110 transition-transform flex-shrink-0">
                                            <step.icon size={24} />
                                        </div>
                                        <div>
                                            <h4 className="font-black text-slate-900 mb-1">{step.title}</h4>
                                            <p className="text-sm text-slate-600 font-bold">{step.desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* RIGHT CONTENT: FORM */}
                        <div className="glass-card p-8 md:p-12 shadow-premium border border-slate-100 animate-fade-in" style={{ animationDelay: '0.2s' }}>
                            <div className="flex items-center gap-4 mb-10 pb-8 border-b border-slate-50">
                                <div className="w-12 h-12 bg-lime-500 text-white rounded-2xl flex items-center justify-center">
                                    <Upload size={24} />
                                </div>
                                <h3 className="text-2xl font-black tracking-tighter text-slate-900">Kirim Resep</h3>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-cyan-500/5 focus:border-cyan-500 transition-all font-bold text-sm text-slate-900 placeholder:text-slate-300"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        />
                                        <div className="w-full h-36 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center group-hover:bg-cyan-500/5 group-hover:border-cyan-500/30 transition-all">
                                            <Camera className="text-slate-300 mb-3 group-hover:text-cyan-500 transition-colors" size={32} />
                                            <p className="text-[10px] font-black text-slate-400 group-hover:text-cyan-600 transition-colors uppercase tracking-[0.2em]">Pilih atau Seret Foto Resep</p>
                                        </div>
                                    </div>
                                ...
                                <button
                                    ...
                                    className="w-full py-5 bg-lime-500 text-white font-black rounded-3xl shadow-xl shadow-lime-500/20 hover:bg-lime-600 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 uppercase tracking-[0.2em] text-xs disabled:opacity-50"
                                >
